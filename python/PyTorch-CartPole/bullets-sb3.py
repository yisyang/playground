...
def start(mode, n_envs, training_steps):
    ai_play_mode = 'ai'
    training_mode = 'train'
    human_mode = 'human'
    ...
    if mode in [training_mode, ai_play_mode]:
        if n_envs > 1:
            # Parallel envs
            env = make_vec_env('rj_gym_envs:bullets-v0', n_envs=n_envs)
        else:
            # Single env
...
class ActionState:
    ...
    def to_array(self):
        return [self.x_action, self.y_action, self.charge_weapon, self.charge_shield]
